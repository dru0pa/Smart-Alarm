    ...
    def getWeather(self):
        if (self.cache is None or self.cacheTimeout is None or self.cacheTimeout < datetime.datetime.now(pytz.timezone('Africa/Johannesburg'))):
            ...
            if (place is None or place is ""):
                place = "Johannesburg, ZA"  # Default to Johannesburg, ZA
            ...
            try:
                ...
                timeout += datetime.timedelta(
                    minutes=1)  # Don't keep the cache for too long, just long enough to avoid request spam
...
class Weather:
    ...
    def speech(self):
        self.speech = ""
        self.speech += "The weather is currently %s. " % (self.condition)
        print("The weather is currently %s. " % (self.condition))
        self.speech += "Temperature %s degrees, " % (self.temp)
        self.speech += "wind %s degrees at %s knots, " % (splitNumber(self.wdir), self.wspeed)
        self.speech += "Q N H %s hectopascals" % (splitNumber(self.pressure))

        engine = pyttsx.init()
        engine.say("The weather is currently %s. " % (self.condition))
        engine.say('Sally sells seashells by the seashore.')

        return self.speech

    def __str__(self):
        return "Weather[temp=%s,wdir=%s,wspeed=%s,press=%s,cond='%s']" % (
            self.temp, self.wdir, self.wspeed, self.pressure, self.condition)
